...
class PurchaseController extends Controller
{
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('purchases-add')){
            ...
            return view('backend.purchase.create', compact('lims_supplier_list', 'lims_agent_list', 'lims_warehouse_list', 'lims_tax_list', 'lims_product_list_without_variant', 'lims_product_list_with_variant', 'currency_list', 'custom_fields'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        foreach ($product_id as $i => $id) {
            ...
            // else {
                ...
                if($product_purchase['product_batch_id']) {
                    ...
                    $lims_product_warehouse_data = Product_Warehouse::where([
                                                ...
                                                ->select('price')
                                                ->first();
                                                // dd('ase',$lims_product_warehouse_data);
                    if($lims_product_warehouse_data)
                        $price = $lims_product_warehouse_data->price;
                    else
                    $price == $price;
                    // dd('adad');
                    // $price = $data['net_unit_cost'][$i] ?? null;
            ...
            ->where('product_warehouse.warehouse_id', $lims_product_warehouse_data->warehouse_id )
            ->where('product_warehouse.product_id',$lims_product_warehouse_data->product_id )
            ->where('product_warehouse.variant_id',$lims_product_warehouse_data->variant_id )
            ->where('product_warehouse.product_batch_id',$lims_product_warehouse_data->product_batch_id )
            ->groupBy(
                'product_warehouse.id',
                'product_warehouse.product_id',
                'products.name',
            ...
            if ($get_all_products) {
                $lims_product_warehouse_data->actual_unit_cost = $get_all_products->actual_cost;
                $lims_product_warehouse_data->save();
            }

// dd('get_all_products',$get_all_products);
