...
class AgentController extends Controller
{
    ...
    public function index()
    {
        ...
        if($role->hasPermissionTo('billers-index')) {
            ...
            $lims_biller_all = Agent::where('is_active', true)->get();
            return view('backend.agent.index',compact('lims_biller_all', 'all_permission'));
        }
        else
    ...
    {
        $lims_biller_data = Agent::find($id);
        $this->fileDelete(public_path('images/biller'),$lims_biller_data->image);

        $lims_biller_data->is_active = false;
        $lims_biller_data->save();
        $this->cacheForget('biller_list');
        return redirect('agent')->with('not_permitted','Data deleted successfully');
    }
}
