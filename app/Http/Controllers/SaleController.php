...
class SaleController extends Controller
{
            ...
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['options'] = '<div class="btn-group">
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">
                                ...
                                <li><a href="'.route('sale.challan', $sale->id).'" class="btn btn-link"><i class="fa fa-copy"></i> '.trans('file.Generate Challan').'</a></li>
                                <li>
    ...
    {
        $role = Role::find(Auth::user()->role_id);
        $lims_agent_list = Agent::where('is_active', true)->get();
        if($role->hasPermissionTo('sales-add')) {
            ...
            return view('backend.sale.create',compact('currency_list', 'companies','all_terms', 'lims_agent_list', 'lims_customer_list', 'lims_warehouse_list', 'lims_biller_list', 'lims_pos_setting_data', 'lims_tax_list', 'lims_reward_point_setting_data','options', 'numberOfInvoice', 'custom_fields', 'lims_customer_group_all'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function getByCompany($companyId)
{
    ...
    public function store(Request $request)
    {
        ...
        try {*/
            ...
            //     $data['commission_value'] = ($data['grand_total'] * $data['commission_value']) / 100;
            // }
            if ($data['order_discount_type'] == 'percentage') {
                $data['order_discount_type'] = ($data['grand_total'] * $data['order_discount_value']) / 100;
            }


            // if ($data['commission_type'] == 'flat') {
            //     $data['commission_value'] = $data['commission_value'];
            ...
            // $mail_data['commission_value'] = $lims_sale_data->commission_value;
            ...
            foreach ($product_id as $i => $id) {
                ...
                if($imei_number[$i] && !str_contains($imei_number[$i], "null") && $data['sale_status'] == 1) {
                    ...
                    // if (count($all_imei_numbers) < 1) {
                    //     $all_imei_numbers = ['N/A'];
                    // }
            ...
            if($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && $data['pos'] && $data['paid_amount'] > 0)) {
                foreach($data['paid_by_id'] as $key=>$value)
                {
                    if($data['paid_amount'][$key] > 0) {
                        ...
                        elseif($paying_method == 'Moneipoint'){
                            $moneipointSaleData = [];
                            $moneipointSaleData['amount'] = $lims_sale_data->paid_amount;
                            $moneipointSaleData['reference_no'] = $lims_sale_data->reference_no;
                            $data = $this->moneipoint($moneipointSaleData);
                            $lims_customer_data->save();
                        }
                    }
                }
    ...
    public function sendWhatsApp(Request $request)
    {
        ...
        // Get status text
        $saleStatus = $saleStatusMap[$lims_sale_data->sale_status] ?? 'Unknown';
        $paymentStatus = $paymentStatusMap[$lims_sale_data->payment_status] ?? 'Unknown';
        $dueAmount = $lims_sale_data->grand_total - $lims_sale_data->paid_amount;
        $message = "🛒 Sale Details\n";
        $message .= "📧 Email: " . $lims_customer_data->email . "\n";
        $message .= "📌 Reference No: " . $lims_sale_data->reference_no . "\n";
        $message .= "📦 Sale Status: " . $saleStatus . "\n";
        ...
        $message .= "💳 Paid Amount: " . number_format($lims_sale_data->paid_amount, 2) . "\n";
        $message .= "🧾 Due Amount: " . number_format($dueAmount, 2) . "\n\n";
    ...
    public function getProduct($id)
    {
        ...
        $lims_product_with_batch_warehouse_data = $query
        ->whereNull('product_warehouse.variant_id')
        ...
        //return $lims_product_with_imei_warehouse_data;
...
// dd($lims_product_with_variant_warehouse_data);
    ...
    public function posSale($id='')
    {
        ...
        if($role->hasPermissionTo('sales-add')) {
            ...
            if(in_array('restaurant',explode(',',$general_setting->modules))){
                return view('backend.sale.pos', compact('service_list','waiter_list','currency_list','role','all_permission', 'lims_customer_list', 'lims_customer_group_all', 'lims_warehouse_list', 'lims_reward_point_setting_data', 'lims_tax_list', 'lims_biller_list', 'lims_pos_setting_data', 'options', 'lims_brand_list', 'lims_category_list', 'lims_table_list', 'lims_coupon_list', 'flag', 'numberOfInvoice', 'custom_fields'));
            }
    ...
    public function recentSale()
    {
        ...
        if(in_array('restaurant',explode(',',$general_setting->modules))){
            if(Auth::user()->role_id > 2 && config('staff_access') == 'own') {
                ...
                $recent_sale = Sale::join('customers', 'sales.customer_id', '=', 'customers.id')->select('sales.id','sales.reference_no','sales.customer_id','sales.grand_total','sales.created_at','customers.name')->where('sale_status', 5)->orderBy('id', 'desc')->take(10)->get();
                return response()->json($recent_sale);
            }
        }
        else {
    ...
    public function productSaleData($id)
    {
        ...
        foreach ($lims_product_sale_data as $key => $product_sale_data) {
            ...
            $general_setting = DB::table('general_settings')->select('modules')->first();
            if(in_array('restaurant',explode(',',$general_setting->modules))){
                $product_sale[10][$key] = json_decode($product_sale_data->topping_id);
            }
        }
        return $product_sale;
    }
    ...
    public function genChallan($id)
    {
        ...
        if($lims_pos_setting_data->invoice_option == 'A4') {
            // dd('first');
            return view('backend.sale.challan', compact('lims_sale_data', 'currency_code', 'lims_product_sale_data', 'lims_biller_data', 'lims_warehouse_data', 'lims_customer_data', 'lims_payment_data', 'numberInWords', 'paid_by_info', 'sale_custom_fields', 'customer_custom_fields', 'product_custom_fields', 'qrText', 'totalDue'));
        }
