            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex align-items-center">
                        <h4>{{trans('file.Add Sale Price')}}</h4>
                    </div>
                    <div class="card-body">
                        <p class="italic"><small>{{trans('file.The field labels marked with * are required input fields')}}.</small></p>
                        {!! Form::open(['route' => 'qty_adjustment.addSaleStore', 'method' => 'post', 'files' => true, 'id' => 'adjustment-form']) !!}
                        <div class="row">
                            <div class="col-md-12">
                                <div class="row">
                                    <div class="col-md-4">
                                            ...
                                            <table id="myTable" class="table table-hover order-list">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th>{{trans('file.Product Batch Id')}}</th>
                                                        <th>{{trans('file.Unit Cost')}}</th>
                                                        <th>{{trans('file.Old Price')}}</th>
...
<script type="text/javascript">
	...
	$('select[name="warehouse_id"]').on('change', function() {
	    ...
	    $.get('getproduct/' + id, function(data) {
	        ...
	        product_qty = data[2];
	        unit_cost = data[3];
	        product_batch_id = data[4];
	        net_unit_cost = data[5];
	        $.each(product_code, function(index) {
	            lims_product_array.push(product_code[index] + ' (' + product_name[index] + ')' + '|' + unit_cost[index] + '=' + product_batch_id[index] + '>' + net_unit_cost[index]);
 productSearch(product_code[index] + ' (' + product_name[index] + ')' + '|' + unit_cost[index] + '=' + product_batch_id[index] + '>' + net_unit_cost[index]);
		...
		$.ajax({
            ...
            success: function(data) {
                ...
                if(flag){
                    ...
                    cols += '<td>' + data[6] + '<input type="hidden" name="price[]" value="'+data[6]+'" /></td>';
