...
class ReportController extends Controller
{
    ...
    public function agentCommissionReport(Request $request)
    {
        ...
        $warehouse_id = $data['warehouse_id'];
        $lims_warehouse_list = Warehouse::where('is_active', true)->get();
        return view('backend.report.agent_commission_report',compact('start_date', 'end_date', 'warehouse_id', 'lims_warehouse_list'));
    }
    public function singleProductReport(Request $request)
    {
        ...
        $warehouse_id = $data['warehouse_id'];
        $lims_warehouse_list = Warehouse::where('is_active', true)->get();
        return view('backend.report.single_product_report',compact('start_date', 'end_date', 'warehouse_id', 'lims_warehouse_list'));
    }
    ...
    public function productReport1(Request $request)
    {
        ...
        return view('backend.report.product_report_data',compact('start_date', 'end_date', 'warehouse_id', 'lims_warehouse_list'));
    }
    ...
    public function agentCommissionReportData(Request $request)
    {
        $data = $request->all();
        $start_date = $data['start_date'];
        $end_date = $data['end_date'];
        ...
        if (!empty($search)) {
            $report_data = array_filter($report_data, function ($row) use ($search) {
                return stripos($row['reference_no'], $search) !== false ||
                    stripos($row['warehouse_name'], $search) !== false ||
                    stripos($row['agent_name'], $search) !== false ||
        ...
        $paginatedData = array_slice($report_data, $start, $length);

        return response()->json([
            'draw' => $draw,
            'recordsTotal' => count($report_data),
            'recordsFiltered' => count($report_data),
            'data' => $paginatedData,
        ]);
    }
    public function singleProductReportData(Request $request)
    {
        $data = $request->all();
        $start_date = $data['start_date'];
        $end_date = $data['end_date'];
        ...
        $start_date = $start_date . ' 00:00:00';
        $end_date = $end_date . ' 23:59:59';
        $query = Product_Sale::whereBetween('created_at', [$start_date, $end_date]);
        if ($warehouseId != 0) {
            $query->where('warehouse_id', $warehouseId);
        }
        $sales = $query->orderBy('created_at', 'desc')->get();
        ...
        foreach ($sales as $sale) {
            $sale_id = Sale::find($sale->sale_id);
            $warehouse = Warehouse::find($sale_id->warehouse_id);
            $product_name = Product::find($sale->product_id);
            $variant = Variant::find($sale->product_id);
            ...
            $report_data[] = [
                ...
                'warehouse_name'    => $warehouse ? $warehouse->name : '',
                'qty'               => $sale->qty,
                'created_at'        => $sale->created_at->format('d F Y'),
        ...
        if (!empty($search)) {
            $report_data = array_filter($report_data, function ($row) use ($search) {
                return stripos($row['product_name'], $search) !== false ||
                    stripos($row['variant_name'], $search) !== false ||
                    stripos($row['created_at'], $search) !== false;
            });
        }
        $draw = intval($request->input('draw'));
    ...
    public function productReportData1(Request $request)
    {
