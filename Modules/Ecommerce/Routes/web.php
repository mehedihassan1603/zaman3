...
use Modules\Ecommerce\Http\Controllers\delivery;
use Modules\Ecommerce\Http\Controllers\WidgetController;
	...
	if(config('database.connections.saleprosaas_landlord')) {
    	...
    	Route::middleware(['ecommerce','customerauth', InitializeTenancyByDomain::class,PreventAccessFromCentralDomains::class])->prefix('customer')->group(function () {
    		...
    		Route::get('/rating', [CustomerController::class, 'rating']);
    		Route::get('/getOrders', [CustomerController::class, 'getOrderProducts'])->name('getOrderProducts');
            Route::post('/submit-rating', [CustomerController::class, 'submitRating'])->name('submitRating');
    	...
    	Route::middleware(['ecommerce'])->group(function () {
            ...
            Route::get('delivery-options/divisions', [delivery::class, 'indddex'])->name('delivery.divisions');

            Route::get('delivery-options/districts/{division_id}', [delivery::class, 'getDistricts'])->name('delivery.districts');
            Route::get('delivery-options/upazillas/{district_id}', [delivery::class, 'getUpazillas'])->name('delivery.upazillas');
