    ...
    public function quotationData(Request $request)
    {
        ...
        if(!empty($quotations))
        {
            foreach ($quotations as $key => $quotation)
            {
                ...
                $getcompany = Company::findOrFail($quotation->customer->company_name);
                // dd($quotation);
                $nestedData['company_name'] = $getcompany ? $getcompany->name : 'N/A' ;
                $nestedData['head'] = $getcompany ? $getcompany->head_office_address : 'N/A' ;
                $nestedData['factory'] = $getcompany ? $getcompany->factory_office_address : 'N/A' ;
                $nestedData['customer'] = $quotation->customer->name;
                $nestedData['phone'] = $quotation->customer->phone_number;
                $nestedData['email'] = $quotation->customer->email;

                $getgroup = Group::findOrFail($getcompany->group_id);
                $getarea = Area::findOrFail($getcompany->area_id);
                $nestedData['company_name'] = $getcompany->name ? $getcompany->name : 'N/A' ;
                $nestedData['group_name'] = $getgroup->name ? $getgroup->name : 'N/A' ;
                $nestedData['area_name'] = $getarea->name ? $getarea->name : 'N/A' ;
$termIds = json_decode($quotation->terms, true) ?? [];

if (!empty($termIds)) {
    $terms = Terms::whereIn('id', $termIds)->pluck('name')->toArray();
    $termString = implode(', ', $terms);
} else {
    $termString = 'N/A'; // or just ''
}
// $nestedData['terms'] = $terms->pluck('name')->implode(', ');
                ...
                $nestedData['options'] .= '<li>
                        <a href="'.route('quotation.create_sale', $quotation->id).'" class="btn btn-link"><i class="fa fa-shopping-cart"></i> '.trans('Create SO').'</a>
                        </li>';
                // $nestedData['options'] .= '<li>
    ...
    public function store(Request $request)
    {
        ...
        foreach ($product_id as $i => $id) {
            ...
            if($sale_unit_id)
                ...
                $mail_data['unit'][$i] = '';
            $lims_product_data = Product::find(id: $id);
...
// dd($lims_product_data);
    ...
    public function productQuotationData($id)
    {
        ...
        foreach ($lims_product_quotation_data as $key => $product_quotation_data) {
            ...
            $product_quotation[8][$key] = $product->attachment;
        }
    ...
    public function createSale($id)
    {
        ...
        $all_terms = Terms::get();
        $termIds = json_decode($lims_quotation_data->terms, true) ?? [];
        // dd('aaaa',$termIds);
...
if (!empty($termIds)) {
    ...
    $termString = 'N/A'; // or just ''
        ...
        return view('backend.quotation.create_sale',compact('lims_customer_list', 'all_terms', 'termIds', 'lims_warehouse_list', 'lims_biller_list', 'lims_tax_list', 'lims_quotation_data','lims_product_quotation_data', 'lims_pos_setting_data', 'termString'));
    }
