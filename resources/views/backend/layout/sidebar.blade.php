        <ul id="side-main-menu" class="side-menu list-unstyled d-print-none">
            ...
            <ul id="product" class="collapse list-unstyled ">
                ...
                @endif--}}
                {{--<li id=""><a href="{{route('stock-count.sale_index')}}">{{trans('Add Sale Price')}}</a></li>--}}
            </ul>
            </li>
            @endif
            <?php
            ...
            <li><a href="#inquiry" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-document"></i><span>{{trans('Inquiry')}}</span><span></a>
                <ul id="inquiry" class="collapse list-unstyled ">
                    <li id="inquiry-list-menu"><a href="{{route('inquiries.index')}}">{{trans('Inquiry List')}}</a></li>

                    <li id="inquiry-create-menu"><a href="{{route('inquiries.create')}}">{{trans('Add Inquiry')}}</a></li>
            ...
            <li class="d-none"><a href="{{route('warrenty.index')}}"> <i class="dripicons-return"></i><span>{{trans('Warrenty')}}</span></a>
            ...
            <ul id="people" class="collapse list-unstyled ">
                ...
                <li id="customer-list-menu"><a href="{{route('customer.index')}}">{{trans('Contact Person List')}}</a></li>
                <?php
                    $customer_add_permission_active = $role_has_permissions_list->where('name', 'customers-add')->first();
                ?>
                @if($customer_add_permission_active)
                <li id="customer-create-menu"><a href="{{route('customer.create')}}">{{trans('Add Contact Person')}}</a></li>
                @endif
                @endif
                ...
                @endif
                @endif
                <li ><a href="{{route('agent.index')}}">{{trans('Agent List')}}</a></li>

                <li ><a href="{{route('agent.create')}}">{{trans('Add Agent')}}</a></li>
            ...
            <ul id="report" class="collapse list-unstyled ">
                ...
                <li>
                    {!! Form::open(['route' => 'report.agentCommissionReport', 'method' => 'get', 'id' => 'agent-commission-form']) !!}
                        <input type="hidden" name="start_date" value="{{ date('Y-m') . '-01' }}" />
                        <input type="hidden" name="end_date" value="{{ date('Y-m-d') }}" />
                        <input type="hidden" name="warehouse_id" value="0" />
                        <a href="#" id="agent-report-link">{{ trans('file.Agent Commission Report') }}</a>
                    {!! Form::close() !!}
                </li>
                <script>
                    document.getElementById('agent-report-link').addEventListener('click', function(e) {
                        e.preventDefault();
                        document.getElementById('agent-commission-form').submit();
                    });
                </script>
                <li>
                    {!! Form::open(['route' => 'report.singleProductReport', 'method' => 'get', 'id' => 'single-product-form']) !!}
                        <input type="hidden" name="start_date" value="{{ date('Y-m') . '-01' }}" />
                        <input type="hidden" name="end_date" value="{{ date('Y-m-d') }}" />
                        <input type="hidden" name="warehouse_id" value="0" />
                        <a href="#" id="single-product-link">{{ trans('file.Specific Product Report') }}</a>
                    {!! Form::close() !!}
                </li>
                <script>
                    document.getElementById('single-product-link').addEventListener('click', function(e) {
                        e.preventDefault();
            ...
            @if(\Auth::user()->role_id != 5)
                @if(!config('database.connections.saleprosaas_landlord'))
                    <li class="d-none"><a href="{{url('addon-list')}}" id="addon-list"> <i class="dripicons-flag"></i><span>{{trans('file.Addons')}}</span></a></li>
                @endif
                ...
                <li class="d-none"><a href="#ecommerce" aria-expanded="false" data-toggle="collapse"> <i class="dripicons-shopping-bag"></i><span>eCommerce</span></a>
