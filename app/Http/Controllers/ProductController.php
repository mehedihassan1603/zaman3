...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        //     dd('Attachment detected:', $request->file('attachment'));
        // } else {
        //     dd('No attachment found', [
        ...
        $request->merge([
            'code' => 'ZML-' . ltrim($request->code, 'ZML-') // avoid double prefixing
        ]);
...
if ($request->hasFile('attachment')) {
    ...
    $lims_product_data->attachment = 'uploads/attachments/' . $filename;
    $lims_product_data->save();
        ...
        elseif(!isset($data['is_initial_stock']) && config('without_stock') == 'yes') {
    ...
    public function getBatches($product_id,$variant_id)
{

    // dd($product_id,$variant_id);
    // dd(gettype($variant_id), (int) $variant_id);

    if ((int) $variant_id !== 0) {  // Corrected the condition
        $batches = DB::table('product_warehouse as pw')
            ->join('product_batches as pb', function ($join) {
                $join->on('pw.product_batch_id', '=', 'pb.id')
                     ->on('pw.product_id', '=', 'pb.product_id');
            })
            ->where('pw.product_id', $product_id)
            ->where('pw.variant_id', $variant_id)
            ->select('pb.batch_no', 'pb.id')
            ->get();
    ...
    ->where('pw.product_id', $product_id)
    // ->where('pw.variant_id', null)
    ->select('pb.batch_no', 'pb.id')
    ->get();
    }
    // dd('aaabbb', $product_id, $variant_id);

    // dd($product_id,$variant_id,$batches);
    ...
    public function checkBatchAvailability($product_id, $batch_no, $warehouse_id)
    {
        ...
        if($product_batch_data) {
            $product_warehouse_data = Product_Warehouse::select('qty', 'price')
            ->where([
            ...
            if($product_warehouse_data) {
                ...
                $data['price'] = $product_warehouse_data->price;
